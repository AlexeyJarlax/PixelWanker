...
tasks.register("uiTestSummary") {
    group = "verification"
    description = "Runs connected debug UI tests and prints QA summary line."
    dependsOn(":app:connectedDebugAndroidTest")

    doLast {
        val reportDir = project(":app").file("build/outputs/androidTest-results/connected")
        val reportFiles = reportDir
            .walkTopDown()
            .filter { it.isFile && it.extension == "xml" }
            .toList()
        ...
        println("[QA][UI][SUMMARY]: $successRate% of the tests were successfully completed")
        println("[QA][UI][SUMMARY]: total=$total, passed=$passed, failed=$failures, errors=$errors, skipped=$skipped")
    }
}
